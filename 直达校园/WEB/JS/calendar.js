...
document.write("<html><body><div id='meizzCalendarLayer' style='position: absolute; z-index: 9999; width: " + (cal_Width+4).toString() + "px; height: 197px; display: none'>");
...
function calendar(obj){
    ...
    if (document.body.clientHeight + dt - t - h >= ch) 
        o.top = (p=="image")? (t + h)+'px' : (t + h + 4)+'px';
    else 
        o.top  = (t - dt < ch) ? ((p=="image")? (t + h)+'px' : (t + h +4)+'px') : (t - ch)+'px';
    if (dw + dl - l >= cw) 
        o.left = l+"px"; 
    else 
...
function cal_writeIframe(){
    ...
    "    <table id=tableDay height='120px' width='"+ cal_Width +"px' border=0 cellspacing=1 cellpadding=0><tbody>";
         ...
         strIframe +="<td colspan="+(42-WebCalendar.dayShow).toString()+" class=out style='text-align:center;' >" + 
         "<input title=清空 style=' background-color: " + WebCalendar.btnBgColor +";cursor: hand; padding-top: 2px; width: 44%; height: 100%;' onfocus='this.blur()'"+
         ...
         " type=button title=关闭 value='关闭' onclick='parent.cal_hiddenCalendar()'>" + 
         "</td></tr></tbody></table>"+
